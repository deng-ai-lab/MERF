...
from model.MERF_evo_v6 import MERF
...
def get_args():
    ...
    parser.add_argument("--run_name", type=str, default="v6")
...
def train_one_epoch(args, evo_model, reward_models, reference_model, optimizer, epoch, total_epoch,
    ...
    entropy_values = []
    elite_losses = []
    for inner_epoch in tqdm(range(args.inner_epochs), desc="CR v6 Training"):
        optimizer.zero_grad()
...
if __name__ == "__main__":
    ...
    loss_dir = os.path.join("logs/evo_cr_v6", f"{time_str}_{args.run_name}", train_dataset.pdb_id)
