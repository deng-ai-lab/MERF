...
from data_process.renumber_abbind import load_residue_mapping
...
def process_pdb(args):
    ...
    if pdb_id in RENUMBER_PDB_IDS:
        wt_dir = wt_dir.replace("PDBs", "PDBs_renumbered")

    mutate_infos = df_pdb['mutant'].tolist()

    mut_list(pdb_id, mutate_infos, wt_dir, fix_dir, mut_dir)
