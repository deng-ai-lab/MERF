...
from model.MERF_evo_v8 import MERF
...
def get_args():
    ...
    parser.add_argument("--eval_sample_count", type=int, default=32)
...
def evaluate_reward_topk_samples(args, evo_model, policy_batch, train_dataset, collate_fn, reward_models, device):
    ...
    with torch.no_grad():
        ...
        eval_acquisition = make_acquisition(
            eval_center, eval_std, args.uncertainty_weight, args.reward_direction
        )
...
def train_one_epoch(args, evo_model, reward_models, reference_model, optimizer, epoch, total_epoch,
    ...
    with torch.no_grad():
        ...
        acquisition = make_acquisition(
            mean_scores, std_scores, args.uncertainty_weight, args.reward_direction
        )
...
if __name__ == "__main__":
    ...
    loss_dir = os.path.join("logs/evo_cr_v8", f"{time_str}_{args.run_name}", train_dataset.pdb_id)
