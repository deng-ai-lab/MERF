...
from model.MERF_evo_v5 import MERF
...
def get_args():
    ...
    parser.add_argument("--run_name", type=str, default="v5")
    ...
    parser.add_argument("--train_mode", choices=["bias_only", "finetune"], default="bias_only")
...
def score_projected_actions(train_dataset, collate_fn, reward_models, projected_actions, device):
    score_batches = [
    ...
    mean_score = score_stack.mean(dim=0)
    std_score = score_stack.std(dim=0, unbiased=False) if score_stack.size(0) > 1 else torch.zeros_like(mean_score)
    return mean_score, std_score, score_stack
...
def train_one_epoch(args, evo_model, reward_models, reference_model, optimizer, epoch, total_epoch,
    ...
    with torch.no_grad():
        ...
        mean_scores, std_scores, score_stack = score_projected_actions(
            train_dataset, collate_fn, reward_models, projected_actions.to(device), device
        )
        acquisition = mean_scores + args.uncertainty_weight * std_scores
    evo_model.train()

    batch_baseline = float(acquisition.mean().item())
    if reward_baseline is None:
        reward_baseline = batch_baseline
    advantage_values = reward_baseline - acquisition.detach()
    ...
    elite_actions = choose_elites(
        acquisition, projected_actions.to(device), args.elite_top_k, args.elite_mode, args.elite_min_hamming
    )
    ...
    entropy_values = []
    elite_losses = []
    for inner_epoch in tqdm(range(args.inner_epochs), desc="CR v5 Training"):
        optimizer.zero_grad()
    ...
    best_sample_idx = int(torch.argmin(acquisition.detach()).item())
    ...
    if epoch % args.eval_interval == 0:
        ...
        with torch.no_grad():
            ...
            greedy_mean, greedy_std, _ = score_projected_actions(
                train_dataset, collate_fn, reward_models, greedy_actions.unsqueeze(0), device
            )
    ...
    print(
        "Epoch:%d/%d PredRank:%s SampleRank:%s GreedyRank:%s BestRank:%s Entropy:%.4f"
        % (
            epoch + 1,
            total_epoch,
            str(pred_best_rank),
...
if __name__ == "__main__":
    ...
    loss_dir = os.path.join("logs/evo_cr_v5", f"{time_str}_{args.run_name}", train_dataset.pdb_id)
