...
v8 保留 SABDAB 的 GRPO、RepairPDB WT、FoldX 突变结构、批处理 reward 评分及
...
V8_POLICY_UPDATE_MODE = "v8_coverage_balanced_action_gather_restore_best"
...
def get_args():
    parser = argparse.ArgumentParser(description="SABDAB v8 GRPO evolution")
    ...
    parser.add_argument("--run_name", type=str, default="v8")
    parser.add_argument("--log_root", type=str, default=str(PROJECT_DIR / "logs/evo_sabdab_v8"))
    ...
    parser.add_argument("--fixed_dir", type=str, default=str(PROJECT_DIR / "data/sabdab/PDBs_fixed_v8"))
    parser.add_argument("--mut_dir", type=str, default=str(PROJECT_DIR / "data/sabdab/PDBs_evo_v8"))
    parser.add_argument(
        "--plm_embedding_path",
        type=str,
        default=str(PROJECT_DIR / "data/sabdab/PLM_embeddings_sabdab_v8.pkl"),
    )
    parser.add_argument("--foldx_workers", type=int, default=8)
    parser.add_argument("--foldx_bin", type=str, default=DEFAULT_FOLDX_BIN)

    # v8 GRPO：统一使用三位点完整动作和 coverage-balanced inner pass。
    ...
    if args.comb_num != 3:
        raise ValueError("SABDAB v8 仅支持 --comb_num=3")
    ...
    # 保留旧实验启动命令中的字段名兼容性；v8 实际语义是完整覆盖 pass。
...
def train_one_epoch(
    ...
    if candidate_pool is None:
        metrics = {
            "epoch": epoch,
            "policy_update_mode": V8_POLICY_UPDATE_MODE,
