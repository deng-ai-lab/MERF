#!/usr/bin/env python
"""Run FoldX and MERF ddG prediction for plectin1 multipoint mutation combinations."""

from __future__ import annotations

import argparse
import os
import pickle
import shutil
import sys
from pathlib import Path

os.environ.setdefault("CUDA_VISIBLE_DEVICES", "2")

import pandas as pd
import torch
from torch.utils.data import DataLoader
from tqdm import tqdm

REPO_ROOT = Path(__file__).resolve().parents[2]
SCRIPT_DIR = Path(__file__).resolve().parent
sys.path.insert(0, str(REPO_ROOT))
sys.path.insert(0, str(SCRIPT_DIR))

from plectin1_common import AA1_TO_AA3, ensure_standard_pdb, find_structure_file, read_structure_records, write_pdb  # noqa: E402
from dataset.dataset_ddg import DDGBaseDataset  # noqa: E402
from model.MERF_v6 import MERF  # noqa: E402
from protein.read_pdbs import PaddingCollate  # noqa: E402
from utils.util import recursive_to, seed_all  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--structure", default="all", help="ptp or all")
    parser.add_argument("--pdb-dir", type=Path, default=SCRIPT_DIR / "pdbs")
    parser.add_argument("--results-dir", type=Path, default=SCRIPT_DIR / "results")
    parser.add_argument("--log-dir", type=Path, default=REPO_ROOT / "logs/pretrain_vm/2026_06_09_16_56_32")
    parser.add_argument("--checkpoint-epoch", type=int, default=200)
    parser.add_argument("--gpu-idx", type=int, default=2)
    parser.add_argument("--batch-size", type=int, default=128)
    parser.add_argument("--foldx-bin", type=Path, default=Path("/home/dataset-local/software_package/foldx_20270131"))
    parser.add_argument(
        "--plm-path",
        type=Path,
        default=Path("/home/dataset-local/projects_dir/pretrained_model/models--facebook--esm2_t33_650M_UR50D"),
    )
    parser.add_argument("--no-foldx", action="store_true")
    return parser.parse_args()


def selected_structures(value: str) -> list[str]:
    if value == "all":
        return ["ptp"]
    return [item.strip() for item in value.split(",") if item.strip()]


def write_multi_mutant_pdb(records: list[dict], row: pd.Series, out_path: Path) -> Path:
    chains = str(row["component_chains"]).split(",")
    resseqs = [int(item) for item in str(row["component_pdb_resseqs"]).split(",")]
    icodes = [item or " " for item in str(row["component_icodes"]).split(",")]
    mts = str(row["component_mts"]).split(",")
    mutation_map = {
        (chain, resseq, icode or " "): AA1_TO_AA3[mt]
        for chain, resseq, icode, mt in zip(chains, resseqs, icodes, mts)
    }
    mutated = []
    for rec in records:
        new_rec = dict(rec)
        key = (rec["chain"], int(rec["resseq"]), rec.get("icode", " ") or " ")
        if key in mutation_map:
            new_rec["resname"] = mutation_map[key]
        mutated.append(new_rec)
    return write_pdb(mutated, out_path)


def prepare_structures(structure_id: str, args: argparse.Namespace, candidates: pd.DataFrame) -> tuple[Path, Path, Path]:
    merf_dir = (args.results_dir / structure_id / "multipoint_merf").resolve()
    wt_dir = merf_dir / "wt_pdbs"
    fix_dir = merf_dir / "fixed_pdbs"
    mut_dir = merf_dir / "mut_pdbs"
    wt_dir.mkdir(parents=True, exist_ok=True)
    fix_dir.mkdir(parents=True, exist_ok=True)
    mut_dir.mkdir(parents=True, exist_ok=True)

    wt_pdb = ensure_standard_pdb(find_structure_file(args.pdb_dir, structure_id), wt_dir / f"{structure_id}.pdb")
    fixed_pdb = fix_dir / f"{structure_id}.pdb"

    used_foldx = False
    if not args.no_foldx:
        if not args.foldx_bin.exists():
            raise FileNotFoundError(f"FoldX executable not found: {args.foldx_bin}")
        old_path = os.environ.get("PATH", "")
        os.environ["PATH"] = f"{args.foldx_bin.parent}:{old_path}"
        from protein.mutate_scripts_foldx import mut_list

        expected = [mut_dir / f"{structure_id}_{mutation}.pdb" for mutation in candidates["pdb_mutation"].tolist()]
        used_foldx = fixed_pdb.exists() and all(path.exists() for path in expected)
        if not used_foldx:
            fixed_pdb.unlink(missing_ok=True)
            for path in expected:
                path.unlink(missing_ok=True)
            mut_list(
                structure_id,
                candidates["pdb_mutation"].tolist(),
                str(wt_dir.resolve()),
                str(fix_dir.resolve()),
                str(mut_dir.resolve()),
            )
            used_foldx = fixed_pdb.exists() and all(path.exists() for path in expected)
        if not used_foldx:
            missing = [str(path) for path in expected if not path.exists()]
            if not fixed_pdb.exists():
                missing.insert(0, str(fixed_pdb))
            raise RuntimeError(f"{structure_id}: FoldX did not create all expected files: {missing[:5]}")

    if not used_foldx:
        shutil.copyfile(wt_pdb, fixed_pdb)
        records = read_structure_records(wt_pdb)
        for _, row in candidates.iterrows():
            write_multi_mutant_pdb(records, row, mut_dir / f"{structure_id}_{row['pdb_mutation']}.pdb")

    return merf_dir, fix_dir, mut_dir


def run_model(input_csv: Path, fix_dir: Path, mut_dir: Path, out_csv: Path, args: argparse.Namespace) -> None:
    with (args.log_dir / "args.pkl").open("rb") as handle:
        train_args = pickle.load(handle)
    train_args.gpu_idx = args.gpu_idx
    seed_all(train_args.seed)
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    model = MERF(train_args).to(device)
    checkpoint = torch.load(args.log_dir / f"Epoch{args.checkpoint_epoch}_MERF.pth", map_location=device)
    model.load_state_dict(checkpoint)
    model.eval()

    dataset = DDGBaseDataset(
        str(input_csv),
        str(fix_dir),
        str(mut_dir),
        knn_num=train_args.knn_neighbors_num,
        knn_agents_num=train_args.knn_agents_num,
        use_plm_embedding=True,
        plm_path=str(args.plm_path),
        plm_embedding_path=str(input_csv.parent / "esm2_650_embeddings.pkl"),
        device=device,
    )
    dataloader = DataLoader(
        dataset,
        batch_size=args.batch_size,
        shuffle=False,
        pin_memory=False,
        drop_last=False,
        num_workers=0,
        collate_fn=PaddingCollate(),
    )

    rows = []
    with torch.no_grad():
        for batch in tqdm(dataloader, desc=f"MERF multipoint {input_csv.parent.parent.name}"):
            batch = recursive_to(batch, device)
            pred, _, _, _ = model(batch, device)
            for pdb_id, mut, score in zip(batch["wt"]["PDB_id"], batch["wt"]["mutate_info"], pred.detach().cpu().view(-1).tolist()):
                rows.append({"PDB_id": pdb_id, "pdb_mutation": mut, "merf_ddg": score})
    pd.DataFrame(rows).to_csv(out_csv, index=False)


def process_structure(structure_id: str, args: argparse.Namespace) -> None:
    combo_dir = args.results_dir / structure_id / "multipoint"
    candidates = pd.read_csv(combo_dir / "multipoint_candidates.csv", keep_default_na=False)
    if candidates.empty:
        raise ValueError(f"No multipoint candidates for {structure_id}")
    merf_dir, fix_dir, mut_dir = prepare_structures(structure_id, args, candidates)
    input_csv = merf_dir / "multipoint_ddg_input.csv"
    pd.DataFrame({"pdb_id": structure_id, "mutant": candidates["pdb_mutation"], "ddG": 0.0}).to_csv(input_csv, index=False)
    merf_csv = merf_dir / "multipoint_merf_ddg_scores.csv"
    run_model(input_csv, fix_dir, mut_dir, merf_csv, args)
    merged = candidates.merge(pd.read_csv(merf_csv)[["pdb_mutation", "merf_ddg"]], on="pdb_mutation", how="left")
    merged = merged.sort_values(["merf_ddg", "combo_size", "combo_combined_rank_sum"])
    merged.to_csv(combo_dir / "multipoint_candidates_with_merf.csv", index=False)
    print(f"{structure_id}: wrote {combo_dir / 'multipoint_candidates_with_merf.csv'} ({len(merged)} rows)")


def main() -> None:
    args = parse_args()
    os.environ["CUDA_VISIBLE_DEVICES"] = str(args.gpu_idx)
    for structure_id in selected_structures(args.structure):
        process_structure(structure_id, args)


if __name__ == "__main__":
    main()
