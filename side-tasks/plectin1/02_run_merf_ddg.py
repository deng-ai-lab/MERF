#!/usr/bin/env python
"""Run MERF ddG prediction for nominated plectin1 peptide mutations."""
...
from plectin1_common import ensure_standard_pdb, find_structure_file, read_structure_records, write_mutant_pdb  # noqa: E402
...
def prepare_structures(structure_id: str, args: argparse.Namespace, candidates: pd.DataFrame) -> tuple[Path, Path, Path]:
    ...
    src_pdb = find_structure_file(args.pdb_dir, structure_id).resolve()
...
def process_structure(structure_id: str, args: argparse.Namespace) -> None:
    ...
    input_csv = merf_dir / "plectin1_ddg_input.csv"
    pd.DataFrame(
        {
            "pdb_id": structure_id,
