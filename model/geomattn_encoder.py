...
from model.common import mask_zero, global_to_local, local_to_global, normalize_vector
...
class GeometricAttention(nn.Module):
    ...
    def _spatial_logits(self, R, t, x):
        ...
        logits_spatial = sum_sq_dist * (
                    (-1 * gamma * np.sqrt(2 / (9 * self.num_query_points))) / 2)  # (N, L, L, n_heads)
    ...
    def _spatial_aggregation(self, alpha, R, t, x):
        ...
        value_points = local_to_global(R, t, value_points.reshape(N, L, self.num_heads, self.num_value_points,
                                                                  3))  # (N, L, n_heads, n_v_pnts, 3)
        aggr_points = alpha.reshape(N, L, L, self.num_heads, 1, 1) * value_points.unsqueeze(
            1)  # (N, *, L, n_heads, n_pnts, 3)
...
class GAEncoder(nn.Module):
    ...
    def encoder_layer_1(self, R, t, p_CB, x, z, mask):
        block = self.blocks[0]
        x = block(R, t, p_CB, x, z, mask)  # Residual connection within the block
        return x

    def encoder_layer_2(self, R, t, p_CB, x, z, mask):
        block = self.blocks[1]
        x = block(R, t, p_CB, x, z, mask)  # Residual connection within the block
        return x

    def encoder_layer_3(self, R, t, p_CB, x, z, mask):
        block = self.blocks[2]
        x = block(R, t, p_CB, x, z, mask)  # Residual connection within the block
        return x
