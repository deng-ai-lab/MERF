...
class DDGBaseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.return_kl_target:
            ...
            batch['rank_target'] = torch.zeros(20, dtype=torch.float32)
            batch['rank_mask'] = torch.zeros(20, dtype=torch.bool)
            batch['has_rank_target'] = torch.tensor(False)
...
class VMDatasetV2(DDGBaseDataset):

    def __init__(self, data_path, fix_dir, mut_dir, knn_num, knn_agents_num,
                use_plm_embedding=False, plm_path=None, plm_embedding_path=None, device='cpu',
                return_rank_target=False
        ):
        self.return_rank_target = return_rank_target
        super(VMDatasetV2, self).__init__(
            data_path, fix_dir, mut_dir, knn_num, knn_agents_num,
            use_plm_embedding, plm_path, plm_embedding_path, device, return_kl_target=False
        )

        if self.return_rank_target:
            self._build_site_ddg_index()

    def _build_site_ddg_index(self):
        self.site_ddg_index = defaultdict(dict)

        for idx in tqdm(range(len(self.data_df)), desc="Building site ddG index (v2)"):
            pdb_id = self.data_df['pdb_id'].iloc[idx]
            pdb_id = pdb_id.replace('+', '').replace('.00', '')
            mutate_info = self.data_df['mutant'].iloc[idx]
            ddG = self.data_df['ddG'].iloc[idx]
            ...
            mut_aa = mutate_info[-1]
            site_key = (pdb_id, position)
            self.site_ddg_index[site_key][mut_aa] = float(ddG)

    def _get_rank_target(self, pdb_id, mutate_info):
        position = str(int(mutate_info[1:-1]))
        site_key = (pdb_id, position)
        site_ddg_dict = self.site_ddg_index.get(site_key, {})

        rank_target = torch.zeros(20, dtype=torch.float32)
        rank_mask = torch.zeros(20, dtype=torch.bool)

        for aa, ddG in site_ddg_dict.items():
            if aa in AA_TO_IDX:
                aa_idx = AA_TO_IDX[aa]
                rank_target[aa_idx] = float(ddG)
                rank_mask[aa_idx] = True

        has_rank_target = rank_mask.sum().item() >= 2
        return rank_target, rank_mask, has_rank_target

    def __getitem__(self, index):
        batch = super(VMDatasetV2, self).__getitem__(index)
        ...
        if self.return_rank_target:
            ...
            rank_target, rank_mask, has_rank_target = self._get_rank_target(pdb_id, mutate_info)
            batch['rank_target'] = rank_target
            batch['rank_mask'] = rank_mask
            batch['has_rank_target'] = torch.tensor(has_rank_target)
