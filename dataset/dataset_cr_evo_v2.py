...
class CREvoDataset(Dataset):
    ...
    def choose_candidate_from_log_probs(self, binary_log_probs):
        ...
        candidate_scores = self.candidate_log_probs(binary_log_probs, reduce='mean')
    ...
    def candidate_log_probs(self, binary_log_probs, reduce='sum'):
        if self.candidate_action_matrix is None:
            raise ValueError(f"No CR candidates loaded for {self.pdb_id}")
    ...
    def project_raw_actions_to_candidates(self, raw_actions, binary_log_probs=None):
        if self.candidate_action_matrix is None:
            raise ValueError(f"No CR candidates loaded for {self.pdb_id}")
        ...
        candidate_actions = self.candidate_action_matrix.to(raw_actions.device)
        distances = (raw_actions.unsqueeze(1) != candidate_actions.unsqueeze(0)).sum(dim=2)
        min_distances = distances.min(dim=1).values

        if binary_log_probs is None:
            candidate_indices = distances.argmin(dim=1)
        else:
            candidate_scores = self.candidate_log_probs(binary_log_probs, reduce='sum')
            masked_scores = candidate_scores.unsqueeze(0).expand(raw_actions.size(0), -1).clone()
            masked_scores = masked_scores.masked_fill(distances != min_distances.unsqueeze(1), float('-inf'))
            candidate_indices = masked_scores.argmax(dim=1)

        projected_actions = candidate_actions[candidate_indices].detach().clone()
        reverse_keys = [self.candidate_reverse_keys[int(idx)] for idx in candidate_indices.detach().cpu().tolist()]
        return projected_actions, reverse_keys, min_distances, candidate_indices
